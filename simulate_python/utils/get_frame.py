...
def get_frame(model, data, opt, scene, context, camera_name, loc_x, loc_y, width=640, height=480):
    ...
    frame = np.zeros((height*width*3, 1), dtype=np.uint8)
    depth = np.zeros(height*width,dtype=np.float32)  # Placeholder for pixel data
    mj.mjr_render(offscreen_viewport, scene, context)
    
    # 4. Read the pixels with mjr_readPixels.
    mj.mjr_readPixels(frame, depth, offscreen_viewport, context)
    ...
    return frame, offscreen_viewport, depth